...
function CreateInterviewForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (numOfQns < 2 || numOfQns > 25) {
      toast.error("Number of questions must be between 2 and 25");
      return;
    }
    ...
    try {
      console.log("Making API call to:", `${server}/api/interview/create-companyInterview`);
      console.log("Sending data:", {role, numOfQns: parseInt(numOfQns), topics: filteredTopics});
      
      const response = await apiClient.post(
        `${server}/api/interview/create-companyInterview`,
        {role, numOfQns: parseInt(numOfQns), topics: filteredTopics}
      );
      ...
      >
        ...
        <div className="mb-4">
          ...
          <input
            ...
            max={25}
            value={numOfQns}
            onChange={(e) => setNumOfQns(parseInt(e.target.value) || 2)}
            className="w-full px-4 py-2 rounded-lg bg-white/10 border border-white/20 placeholder-gray-300 text-white focus:outline-none focus:ring focus:ring-blue-400"
            required
          />
        </div>
